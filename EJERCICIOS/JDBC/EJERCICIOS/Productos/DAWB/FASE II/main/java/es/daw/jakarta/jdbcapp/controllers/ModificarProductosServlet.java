...
//            // El forward de un RequestDispatcher no corta la ejecución del método, simplemente envía la respuesta al cliente.
//            // Para detener el flujo
//            return; // para que no siga ejecutando el resto del método
//        }
//
//        log.info("************** PASO!!!");// RARUNO RARUNO... PASA!!!!
        ...
        try {
            ...
            //productos.sort((p1, p2) -> p1.getCodigo().compareTo(p2.getCodigo())); // ordenación por código ASC
            productos.sort((p1, p2) -> p2.getCodigo().compareTo(p1.getCodigo())); // ordenación por código DESC
            ...
            request.setAttribute("error", "Formato numérico no corrcto. Y ojo con el precio que puede llevar decimales con punto! ej: 1500.80");
            getServletContext().getRequestDispatcher("/error.jsp").forward(request, response);
            // El forward de un RequestDispatcher no corta la ejecución del método, simplemente envía la respuesta al cliente.
            // Para detener el flujo
            return; // para que no siga ejecutando el resto del método
...
//            out.println("<h1> Operación ["+operacion+"] con producto realizada correctamente!!!</h1>");
//            out.println("</body>");
//        }
